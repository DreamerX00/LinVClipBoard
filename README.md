...
```bash
# Download linvclipboard_3.2.2-1_amd64.deb and SHA256SUMS, then verify:
sha256sum --ignore-missing -c SHA256SUMS

# Install (apt resolves dependencies automatically):
sudo apt install ./linvclipboard_3.2.2-1_amd64.deb
```
...
```bash
sha256sum --ignore-missing -c SHA256SUMS
sudo dnf install ./linvclipboard-3.2.2-1.x86_64.rpm
```
...
```bash
sha256sum --ignore-missing -c SHA256SUMS
tar xzf linvclipboard-3.2.2-linux-x86_64.tar.gz
cd linvclipboard-3.2.2-linux-x86_64
